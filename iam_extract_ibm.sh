...
# V9.4.3 2017-02-09 # Balagopal Kaluri  implemented restriction for "-uat" flag, which is not possible to use among with "-centrify" and "vintela" switches.
# V9.4.3 2017-02-13 # Balagopal Kaluri  Fixed non-privilege IDs are extract as privilege ID.
#===============================================================================
...
function formatDate
{
  ...
  declare DD=$2
  declare YY=$3
  set -A mnames 'dummy' 'Jan' 'Feb' 'Mar' 'Apr' 'May' 'Jun' 'Jul' 'Aug' 'Sep' 'Oct' 'Nov' 'Dec'  # Jan -> 1
  MMM=${mnames[$MM]}
...
function parse_shadow
{
  ...
  do
    ...
    if [[ $passwd_changed = "0" ]]; then
      AStore PWChg_Arr ${username} "01 Jan 1970"
    else
  ...
  if [[ $username != "" ]]; then
    ...
    if [[ $maxage = "0" || $maxexpired = "-1" ]]; then
      ...
      if [[ $LastUpdate != "" ]]; then
        ...
        AStore PWExp_Arr ${username} "01 Jan 1970"
      fi  
    fi  
 fi      
}
...
function find_last_alias
{
  ...
  if [[ $_subalias != "" ]]; then
    ...
    do
      ...
      (( Last_Alias_index+=1 ))
      if [[ $DEBUG -ne 0 ]]; then
        echo "find_last_alias: Index $Last_Alias_index"
        APrintAll Last_Alias ":"
      fi
      find_last_alias $_tempAlias
    done
  fi
  IFS=$oFS
}
